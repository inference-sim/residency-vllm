...
class OpenAIServingChat(OpenAIServing):
    ...
    async def _create_chat_completion(
        ...
        for i, engine_input in enumerate(engine_inputs):
            ...
            # Residency patch: accept tenant_id from the
            # x-gateway-inference-fairness-id header (used by blis observe)
            # in addition to the vllm_xargs body field.
            if raw_request is not None and not isinstance(
                sampling_params, BeamSearchParams
            ):
                fairness_id = raw_request.headers.get(
                    "x-gateway-inference-fairness-id"
                )
                if fairness_id:
                    if sampling_params.extra_args is None:
                        sampling_params.extra_args = {}
                    sampling_params.extra_args.setdefault(
                        "tenant_id", fairness_id
                    )
